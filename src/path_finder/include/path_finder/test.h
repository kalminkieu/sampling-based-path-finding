...
namespace path_plan
{
  ...
  public:
    ...
    BRRT_Optimize(const ros::NodeHandle &nh, const env::OccMap::Ptr &mapPtr) : nh_(nh), map_ptr_(mapPtr)
    {
      ...
      nh_.param("BRRT_Optimize/sampling_radius", sampling_radius_, 3.0);
    ...
    bool brrt_optimize(const Eigen::Vector3d &s, const Eigen::Vector3d &g)
    {
      ...
      for (int idx = 0; idx < 10000; ++idx)
      {
        ...
        if (dis(gen) < brrt_optimize_p_)
        {
          ...
          this->samplingOnce(q_rand, true, sampling_radius_);
          ...
          Eigen::Vector3d q_new = map_ptr_->getFreeNodeInLine(nearest_nodeS->x, q_rand, steer_length_);
  ...
  public:
    ...
    void samplingOnceInRadius(Eigen::Vector3d &sample, const Eigen::Vector3d &center, double radius)
    {
      ...
      while (true)
      {
        Eigen::Vector3d offset(dis(gen), dis(gen), dis(gen));
        if (offset.norm() <= 1.0)
        {
          offset.normalize();  // hướng ngẫu nhiên
          double scale = std::cbrt(std::abs(dis(gen))) * radius;  
          offset *= scale;
    ...
    void samplingOnce(Eigen::Vector3d &sample, bool isValidProb, double radius)
    {
      if(isValidProb)
      {
        samplingOnceInRadius(sample, s_guide->x, radius);
      }
      else
      {
        sampler_.samplingOnce(sample, isValidProb);
      }
      if (!preserved_samples_.empty()) {
          static int i = 0;
          sample = preserved_samples_[i];
          i = (i + 1) % preserved_samples_.size();
      }
    }
